...
[bold]llm-harness[/bold] 命令：
  ...
  /clear                清空当前会话消息
  /fork [标题]          复制当前会话为分支
  /load <id> [N]        载入另一会话继续对话（可只显示最近 N 条）
...
AUTO_SEARCH_PROMPT = (
    "\n\n[自动联网搜索规则]\n"
    "默认先联网检索再回答。除一眼可得的简单逻辑、基础数学计算、纯常识和不依赖外部事实的个人建议外，"
    "其余问题都必须先搜索核实，不要仅凭记忆作答或编造。\n"
    "必须搜索的范围包括但不限于：人物、公司、产品、软件、技术、概念、历史、地理、数据、法规、医学、学术、"
    "新闻、价格、天气、版本、事件、比较、推荐，以及任何可能有事实错误、遗漏、争议或时效变化的问题。"
    "用户没有明确要求搜索，也按此规则执行。\n"
    "需要搜索时，在回答第一行只输出 [SEARCH: 搜索词]，不要输出其他文字、解释或结论，然后停止。"
    "搜索词应简洁、具体，优先覆盖问题的核心事实；harness 会把结果作为[网络搜索结果]注入后续上下文。\n"
    "收到搜索结果后，先判断资料是否足够。资料不足、来源相互矛盾、需要补充关键事实或需要交叉验证时，"
    "可以继续用不同且更有针对性的关键词搜索；最多可自动搜索 5 次。资料足够时必须直接作答，不要为了凑次数继续搜索。\n"
    "最终回答应基于已注入的结果给出清晰结论；涉及可核验事实时附上关键来源链接。"
    "若搜索仍无法确认，应明确说明不确定点与缺少的信息。"
)
...
class App:
    ...
    def _show_header(self, title=None):
        """在界面头部显示当前会话标题与模型信息。"""
        t = title or (self.conv["title"] if self.conv else "untitled")
        self.console.print(f"[bold]llm-harness v{__version__}[/bold]  会话 [cyan]{t}[/cyan]  "
                           f"模型 [cyan]{self.resolve_model()}[/cyan]  思考 [cyan]{self.resolve_thinking()}[/cyan]")
    ...
    async def send(self, text, images, files=None):
        ...
        max_searches = MAX_AUTO_SEARCHES
        search_count = 0
        seen_queries = set()
        ...
        while True:
            ...
            if error:
                ...
                self.console.print(f"[yellow]已完成 {max_searches} 轮搜索，但模型未能基于结果生成回答。请切换模型后重试，或使用 /web 查看原始结果。[/yellow]")
            else:
                if assistant_text:
                    self.db.add_message(self.conv["id"], "assistant", [{"type": "text", "text": assistant_text}])
            break
    ...
    def cmd_load(self, args):
        ...
        if cid is None:
            return True
        if cid == self.conv["id"]:
            self.console.print("[yellow]已在当前会话[/yellow]")
            return True
        n = None
        if len(args) > 1:
            try:
        ...
        self.refresh()
        self._show_header()
        self.console.print(f"[green]已载入会话：{conv['title']}[/green]")
        self.render_history(limit=n)
        return True

    def render_history(self, limit=None):
        """重绘当前会话的用户/助手消息（跳过 system 摘要与联网注入）。"""
        shown = 0
        rows = self.db.get_messages(self.conv["id"])
        if limit:
            rows = rows[-limit:]
        for r in rows:
